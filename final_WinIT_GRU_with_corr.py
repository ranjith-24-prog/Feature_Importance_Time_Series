...
def train_model(model, X_train, y_train, optimizer, criterion, num_epochs=20):
...
# Main
def process_dataset_with_winit_gru_with_corr(path, prepare_data_func):
    # Prepare data
    ...
    model = models_final.GRUModel(input_size, hidden_size, output_size, num_layers)
    ...
    csv_output_path = f"output/FI_Dataframes/WinIT/{dataset_name}_WinIT_GRU_WithCorr_without_redundancy.csv"
    os.makedirs(os.path.dirname(csv_output_path), exist_ok=True)
    importance_df.to_csv(csv_output_path, index=False)

    # Plot feature importance
    plot_path = f"output/FI_Plots/WinIT/{dataset_name}_WinIT_GRU_WithCorr_without_redundancy.png"
