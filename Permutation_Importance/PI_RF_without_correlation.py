...
def process_dataset_with_PI_RF_without_corr(path, prepare_data_func):
    # Prepare the data
    ...
    csv_output_path = f"output/FI_Dataframes/Permutation/{dataset_name}_PI_RF_without_corr.csv"
    ...
    plot_path = f"output/FI_Plots/Permutation/{dataset_name}_PI_RF_without_corr.png"
    plt.figure(figsize=(12, 6))
    plt.barh(importance_df['Feature'], importance_df['Importance'], color='skyblue')
    plt.xlabel('Importance')
    plt.ylabel('Feature')
    plt.title('Feature Importance (Permutation Importance with RF)')
